# Session 1.1 CUDA Programming Model
...
实际问题中数据规模通常大于一个 block，一个 block 线程数量有限，因此需要多个 block。线程全局编号：

$$ index =  blockIdx \times blockDim + threadIdx $$
...
下图代码中 betterKernel 尽量使得所有线程走相同的分支，即数据分布于 warp 边界对齐。badKernel 中，偶数下标走 sqrt，奇数下标走 exp，因此会出现之前所说的一个 warp 中有多个分支，导致执行效率下降。
...
__restrict__ 修饰符表示通过这个指针访问的内存和别的变量没有关系，指针之间不会重叠

一个 block 负责计算 C 的一个 32×32 tile；一个 thread 负责这个 tile 里的一个 Cij 整个 block 分批把 A、B 的数据搬进 shared memory，然后重复做乘加。
...
Occupancy 是活动的 warp 数量与 SM 最大支持的 warp 数量​的比值（0-1之间）。Occupancy 表征了 GPU 程序的实际使用率。高 Occupancy 可以隐藏 Memory Latency、提高计算资源利用率，并减少等待时间。
